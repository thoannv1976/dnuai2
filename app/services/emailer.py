...
def email_submit_confirmation(store, user: dict, summary: dict) -> None:
    ...
    body = (
        ...
        f"Hệ thống DNU AI-Assess xác nhận đã tiếp nhận hồ sơ của thầy/cô lúc {now_vn():%H:%M ngày %d/%m/%Y}.\n"
        f"Các mục cần lưu ý bổ sung trước hạn 17h00 ngày 30/6/2026:\n{missing}\n\n"
        "Thầy/cô có thể nộp lại nhiều lần trước hạn; hệ thống chỉ chấm bản nộp cuối cùng.\n\n"
        "Trân trọng,\nBan tổ chức Chương trình đánh giá năng lực AI – DNU 2026"
    )
    send_email(store, user["email"], "[DNU AI-Assess] Xác nhận nộp hồ sơ thành công", body, kind="confirm")
...
def email_reminder(store, user: dict, warnings: list[str]) -> None:
    ...
    body = (
        f"Kính gửi {user.get('ho_ten', user['email'])},\n\n"
        "Còn chưa đầy 24 giờ là đến hạn nộp hồ sơ (17h00 ngày 30/6/2026). "
        "Hồ sơ của thầy/cô còn thiếu các mục sau:\n"
        f"{miss}\n\nĐề nghị thầy/cô hoàn thiện trên hệ thống DNU AI-Assess trước hạn.\n\n"
        "Trân trọng,\nBan tổ chức"
    )
    send_email(store, user["email"], "[DNU AI-Assess] Nhắc hoàn thiện hồ sơ trước hạn 24 giờ", body, kind="reminder")
...
def email_result(store, user: dict, total: float, label: str, detail_lines: list[str]) -> None:
    ...
    body = (
        f"Kính gửi {user.get('ho_ten', user['email'])},\n\n"
        f"Kết quả đánh giá năng lực ứng dụng AI năm 2026 của thầy/cô:\n"
    ...
    send_email(store, user["email"], "[DNU AI-Assess] Kết quả đánh giá năng lực ứng dụng AI 2026", body, kind="result")
